...
def getRewards(env, next_states, t):
    rewards = np.zeros((next_states.shape[0], 1))
    for i in range(len(rewards)):
        rewards[i] = env.rewardFunction(next_states[i], t, isPoint=True)
...
def valueIteration(env):
    ...
    value_table = np.array(np.meshgrid(lx, ly, t)).T.reshape(-1,3)
    ...
    #while(max_mod > 10):
        ...
        for i in range(len(value_table)):
            ...
            rewards = getRewards(env, next_states, value_table[i][2] + 1)
            ...
            value_table[i][4] = np.max(value_candidates)


        modification = value_table[:, 3] - value_table[:, 4] 
        modification = np.fabs(modification)
        max_mod = np.max(modification)
        max_mods.append(max_mod)
